...
timer_interrupt (struct intr_frame *args UNUSED) { 
  ...
  if(thread_mlfqs) {
    ...
    if(timer_ticks () % 4 == 0) {
      ...
      thread_foreach (&thread_update_priority_mlfqs, (void *) &(update_time));
    }
  }
  ...
  while (!list_empty(&sleeping_threads_list)) { //checking whether any thread(s) need to be unblocked
    ...
    if (head_thread->wakeup_ticks <= timer_ticks()) { //comparing the head thread tick time 
      thread_unblock (head_thread->thread);           //with the current tick time and unblocking it if needed
